...
import shutil
from resources.serializers import podcastparser
import zipfile
...
class PodCastModel(db.Model):
    ...
    input_parser = podcastparser
    
    def __init__(self,audio_id,name, duration, upload_time,host,participants,file_path):
    ...
    def save_update_file(cls,audio_id):
        ...
        target = "/".join([app.config['PODCAST_UPLOAD_FOLDER'],str(audio_id)])
    ...
    def delete_file(cls,audio_id):
        shutil.rmtree("/".join([app.config['PODCAST_UPLOAD_FOLDER'],str(audio_id)]),ignore_errors=True)

    @classmethod
    def download_all(cls):
        target_dir = app.config['PODCAST_UPLOAD_FOLDER']
        foldername = 'AudioFiles/archive/podcast.zip'
