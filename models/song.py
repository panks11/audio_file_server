...
import shutil
import datetime
from resources.serializers import songparser
import zipfile
...
class SongModel(db.Model):
    ...
    input_parser = songparser
     
    def __init__(self,audio_id,name, duration, upload_time,file_path):
    ...
    def json(self):
        return {'audio_id': self.audio_id, 'name': self.name, 'duration': self.duration,'upload_time':str(self.upload_time),'file_path':self.file_path}
    ...
    def save_update_file(cls,audio_id):
        ...
        target = "/".join([app.config['SONG_UPLOAD_FOLDER'],str(audio_id)])
    ...
    def delete_file(cls,audio_id):
        shutil.rmtree("/".join([app.config['SONG_UPLOAD_FOLDER'],str(audio_id)]),ignore_errors=True)

    @classmethod
    def download_all(cls):
        target_dir = app.config['SONG_UPLOAD_FOLDER']
        foldername = 'AudioFiles/archive/song.zip'
